...
def train_epoch(
    epoch, model, optimizer, train_loader, writer, args, stock2concept_matrix=None
):
...
def test_epoch(
    epoch, model, test_loader, writer, args, stock2concept_matrix=None, prefix="Test"
):
    ...
    for i, slc in tqdm(
        test_loader.iter_daily(), desc=prefix, total=test_loader.daily_length
    ):
...
def create_loaders(args):
    ...
    df_train["market_value"] = df_train["market_value"].fillna(
        df_train["market_value"].mean()
    )
    df_train["stock_index"] = 733
    df_train["stock_index"] = (
        df_train.index.get_level_values("instrument")
        .map(stock_index)
        .fillna(733)
        .astype(int)
    )
    ...
    df_valid["market_value"] = df_valid["market_value"].fillna(
        df_train["market_value"].mean()
    )
    df_valid["stock_index"] = 733
    df_valid["stock_index"] = (
        df_valid.index.get_level_values("instrument")
        .map(stock_index)
        .fillna(733)
        .astype(int)
    )
    ...
    df_test["market_value"] = df_test["market_value"].fillna(
        df_train["market_value"].mean()
    )
    df_test["stock_index"] = 733
    df_test["stock_index"] = (
        df_test.index.get_level_values("instrument")
        .map(stock_index)
        .fillna(733)
        .astype(int)
    )
...
def main(args):
    ...
    for times in range(args.repeat):
        ...
        if args.model_name == "SFM":
            ...
            model = get_model(args.model_name)(
                args.d_feat, args.hidden_size, args.num_layers, args.dropout, "LSTM"
            )
        elif args.model_name == "Transformer":
            model = get_model(args.model_name)(
                args.d_feat, args.hidden_size, args.num_layers, dropout=0.5
            )
        elif args.model_name == "HIST":
            model = get_model(args.model_name)(
                d_feat=args.d_feat, num_layers=args.num_layers, K=args.K
            )
        else:
            model = get_model(args.model_name)(
                d_feat=args.d_feat, num_layers=args.num_layers
            )
        ...
        for epoch in range(args.n_epochs):
            ...
            torch.save(
                optimizer.state_dict(), output_path + "/optimizer.bin.e" + str(epoch)
            )
            ...
            pprint(
                "train_loss %.6f, valid_loss %.6f, test_loss %.6f"
                % (train_loss, val_loss, test_loss)
            )
            pprint(
                "train_score %.6f, valid_score %.6f, test_score %.6f"
            ...
            pprint(
                "train_ic %.6f, valid_ic %.6f, test_ic %.6f"
                % (train_ic, val_ic, test_ic)
            )
            pprint(
                "train_rank_ic %.6f, valid_rank_ic %.6f, test_rank_ic %.6f"
        ...
        pprint("save info...")
        writer.add_hparams(
            vars(args), {"hparam/" + key: value for key, value in res.items()}
        )
    ...
    pprint(
        ("IC: %.4f (%.4f), Rank IC: %.4f (%.4f)")
        % (
            np.array(all_ic).mean(),
            np.array(all_ic).std(),
            np.array(all_rank_ic).mean(),
            np.array(all_rank_ic).std(),
        )
    )
    ...
    for k in range(len(N)):
        pprint(
            ("Precision@%d: %.4f (%.4f)") % (N[k], precision_mean[k], precision_std[k])
        )
